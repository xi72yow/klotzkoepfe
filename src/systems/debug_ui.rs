...
fn entries() -> Vec<Entry> {
    vec![
        Entry { label: "Player Speed",     get: |s| s.player_speed,           set: |s,v| s.player_speed = v,           step: 10.0,  min: 50.0,  max: 2000.0 },
        Entry { label: "Player HP",        get: |s| s.player_hp,              set: |s,v| s.player_hp = v,              step: 10.0,  min: 10.0,  max: 5000.0 },
        Entry { label: "Zombie Speed",     get: |s| s.zombie_speed,           set: |s,v| s.zombie_speed = v,           step: 5.0,   min: 10.0,  max: 1000.0 },
        Entry { label: "Zombie HP",        get: |s| s.zombie_hp,              set: |s,v| s.zombie_hp = v,              step: 5.0,   min: 5.0,   max: 2000.0 },
        Entry { label: "Zombie Damage",    get: |s| s.zombie_damage,          set: |s,v| s.zombie_damage = v,          step: 1.0,   min: 1.0,   max: 500.0 },
        Entry { label: "Zombie Dmg CD",    get: |s| s.zombie_damage_cooldown, set: |s,v| s.zombie_damage_cooldown = v, step: 0.1,   min: 0.1,   max: 10.0 },
        Entry { label: "Combo Drain",      get: |s| s.combo_drain_speed,      set: |s,v| s.combo_drain_speed = v,      step: 0.01,  min: 0.01,  max: 5.0 },
        Entry { label: "Combo Kill Boost", get: |s| s.combo_kill_boost,       set: |s,v| s.combo_kill_boost = v,       step: 0.05,  min: 0.05,  max: 20.0 },
        Entry { label: "Wave Base",        get: |s| s.wave_base_zombies as f32, set: |s,v| s.wave_base_zombies = v as u32, step: 1.0, min: 1.0, max: 500.0 },
        Entry { label: "Wave Increment",   get: |s| s.wave_zombie_increment as f32, set: |s,v| s.wave_zombie_increment = v as u32, step: 1.0, min: 0.0, max: 100.0 },
        Entry { label: "Spawn Interval",   get: |s| s.spawn_interval,         set: |s,v| s.spawn_interval = v,         step: 0.1,   min: 0.1,   max: 30.0 },
        ...
        w!("Pistol Dmg",    pistol.damage,        1.0,  1.0, 2000.0),
        ...
        w!("Shotgun Dmg",   shotgun.damage,       1.0,  1.0, 500.0),
        w!("Shotgun Pellets",shotgun.pellet_count,1.0,  1.0, 100.0),
        ...
        w!("Flame Dmg",    flamethrower.damage,       0.5,  0.5, 200.0),
        ...
        w!("Grenade Dmg",  grenade.damage,         5.0,  5.0, 5000.0),
        w!("Grenade Range",grenade.range,         25.0, 50.0, 5000.0),
        ...
        w!("Rail Dmg",     railgun.damage,         5.0,  5.0, 5000.0),
        ...
        w!("Freeze Dmg",   freezegun.damage,       0.5,  0.5, 200.0),
        w!("Freeze Slow",  freezegun.slow_factor,  0.05, 0.05, 0.99),
        ...
        w!("Saw Dmg",      buzzsaw.damage,         1.0,  1.0, 500.0),
        ...
        w!("Tesla Dmg",    tesla.damage,           1.0,  1.0, 1000.0),
        w!("Tesla Chains", tesla.chain_count,      1.0,  0.0, 50.0),
        ...
        w!("Mine Dmg",     mine.damage,            5.0,  5.0, 5000.0),
        ...
        w!("Boom Dmg",     boomerang.damage,       1.0,  1.0, 1000.0),
        ...
        w!("Rocket Dmg",   rocket.damage,          5.0,  5.0, 5000.0),
        w!("Rocket Range", rocket.range,          25.0, 50.0, 5000.0),
        ...
        w!("Laser Dmg",    laser.damage,           0.5,  0.5, 500.0),
...
pub fn settings_render(
    ...
    let mut lines = vec![
        ...
        "Up/Down: Setting waehlen | Left/Right: Wert aendern | Shift: 10x".to_string(),
    ...
    for i in visible_start..visible_end {
        ...
        if entry.label == "Friendly Fire" {
            let state = if val >= 0.5 { "ON" } else { "OFF" };
            lines.push(format!("{}{}: {}", marker, entry.label, state));
        } else if entry.label.ends_with("Fuse") {
            lines.push(format!("{}{}: {:.2}s (Range/Speed)", marker, entry.label, val));
        } else {
