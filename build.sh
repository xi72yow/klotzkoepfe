...
BUILD_IMAGE="localhost/klotzkoepfe-build:latest"

echo "Building klotzkoepfe in Podman container..."

podman build -t "$BUILD_IMAGE" -f "$SCRIPT_DIR/Containerfile" "$SCRIPT_DIR"

# .deb aus Container extrahieren
mkdir -p "$OUT_DIR"
CONTAINER_ID=$(podman create "$BUILD_IMAGE")
podman cp "$CONTAINER_ID:/build/target/debian/." "$OUT_DIR/"
podman rm "$CONTAINER_ID" > /dev/null

# Build-Image aufraeumen
